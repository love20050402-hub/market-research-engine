...
def collect_issues(repository, logger, *, opener=None):
    ...
    try:
        for page in (1, 2):
            url = f'https://api.github.com/repos/{repository}/issues?state=open&sort=updated&direction=desc&per_page=100&page={page}'
            ...
            for issue in payload:
                try:
                    row = parse_issue(issue, repository.split('/')[0])
                    if row:
                        records.append(row)
                except (ValueError, TypeError, AttributeError):
                    pending.append({'url': canonical_url(issue.get('html_url', '')) if isinstance(issue, dict) else '', 'reason': 'Issue intake malformed; edit URL:/TEXT: fields (max 20,000 chars)'})
            if len(payload) < 100:
                break
            if page == 2:
                stats.append('Issues: capped at 200 open entries; close reviewed Issues to expose older intake')
